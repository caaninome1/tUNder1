<template>
  <div class="card text-start">
    <div class="card-body">
      <h4 class="card-title">{{ getProfile.name }}</h4>
      <h4 class="card-title">{{ getProfile.age }}</h4>
      <p class="card-text">{{ getProfile.description }}</p>
      <p class="card-text">
        <span class="fw-bold">Género: </span>{{ getProfile.gender }}
      </p>
      <p class="card-text">
        <span class="fw-bold">City: </span>{{ getProfile.city }}
      </p>
      <p class="card-text">
        <span class="fw-bold">Campus: </span>{{ getProfile.campus }}
      </p>
      <p class="card-text">
        <span class="fw-bold">Programa: </span>{{ getProfile.academicProgram }}
      </p>
      <ul class="list-group list-group-flush">
        <li
          v-for="item in getProfile.characteristics"
          :key="item.type"
          class="list-group-item"
        >
          <span class="fw-bold">{{ item.type }}: </span>{{ item.content }}
        </li>
      </ul>
    </div>
  </div>
